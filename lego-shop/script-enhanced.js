/**
 * LEGO Shop Enhanced JavaScript
 * Features: Smooth animations, better UX, mobile optimizations
 * Created: 2026-02-08
 */
...
function normalizeCategory(cat){
  ...
  if(s === 'minifig' || s === 'minifigure' || s === 'minifigures' || s === 'minifigure(s)' || s === 'minifigureseries'){
    return 'Minifigure';
  }
...
function scheduleRender(){
  ...
  RENDER_TIMER = window.setTimeout(()=>{
    ...
    render(ITEMS, state);
  }, 150); // Slightly longer debounce for smoother UX
}
...
function render(items, state){
  ...
  for(const x of out){
    ...
    card.setAttribute('aria-label', `View ${x.name}`);
  ...
  // Update current filters for tracking
...
async function init(){
  ...
  try {
    ...
    // Event wiring with debounced listeners
    ...
    console.log(`LEGO Shop loaded in ${loadTime.toFixed(0)}ms`);
    
  } catch(err){
    console.error(err);
    showToast('Error loading listings. Please use a local server.', 'error');
